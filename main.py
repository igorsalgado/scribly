...
def build_pipeline(whisper: WhisperTranscriber, hf_token: str) -> TranscriptionHandler:
    """Factory Method: wires the Chain of Responsibility with concrete Strategy implementations."""
...
def main() -> None:
    ...
    if not hf_token:
        console.print(
            ...
            "[dim]Configure no arquivo .env. Consulte o README para instruções.[/dim]"
        )
        return

    whisper = WhisperTranscriber(model_size=os.getenv("WHISPER_MODEL", "medium"))
    ...
    if args.file:
        ...
        meeting = use_case.execute(wav_path=args.file)
    else:
